...
docker run -it -p 7860:7860 vincentqin/image-matching-webui:latest python -m imatchui.app --server_name "0.0.0.0" --server_port=7860
```
...
``` bash
python -m imatchui.api.server
```
 
### Run demo
``` bash
python -m imatchui.app
```
...
I provide an example to add local feature in [imatchui/hloc/extractors/example.py](imatchui/hloc/extractors/example.py). Then add feature settings in `confs` in file [imatchui/hloc/extract_features.py](imatchui/hloc/extract_features.py). Last step is adding some settings to `matcher_zoo` in file [imatchui/ui/config.yaml](imatchui/ui/config.yaml).
